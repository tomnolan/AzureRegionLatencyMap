$regions = @"
Location,Type,DisplayName,RegionalDisplayName,RegionType,RegionCategory,Geography,GeographyGroup,Longitude,Latitude,PhysicalLocation,PairedRegion,Special Access,AvailabilityZoneSupport,AvailabilityZoneCount
...
japaneast,Region,Japan East,(Asia Pacific) Japan East,Physical,Recommended,Japan,Asia Pacific,139.77,35.68,Tokyo, Saitama,japanwest,,TRUE,3
...
$regionscsv = ConvertFrom-Csv $regions
$latencycsv = ConvertFrom-Csv $latency

$latencyjson = New-Object System.Collections.ArrayList

foreach ($row in $latencycsv)
{
    $obj = [pscustomobject]@{}
    $null = $obj | Add-Member -MemberType NoteProperty -Name "Source" -Value $row.Source
    $destinations = [ordered]@{}
    $row.psobject.Properties | Where-Object MemberType -eq NoteProperty | ForEach-Object {
        if ($row.$($_.Name) -and $row.$($_.Name) -ne "" -and $_.Name -ne "Source")
        {
            $null = $destinations.Add($_.Name, $row.$($_.Name))
        }
    }
    $null = $obj | Add-Member -MemberType NoteProperty -Name "Destinations" -Value $Destinations
    $null = $latencyjson.Add($obj)
}

$latencyjson | ConvertTo-Json